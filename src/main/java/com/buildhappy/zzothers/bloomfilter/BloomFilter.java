package com.buildhappy.zzothers.bloomfilter;
...
public class BloomFilter {
    ...
    {
        for(SimpleHash f : func)//将字符串value哈希为8个(种子的个数)或多个整数，然后在这些整数的bit上变为1
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < seeds.length; i++) {
            func[i] = new SimpleHash(DEFAULT_SIZE, seeds[i]);//获取每一个种子的hash值
        }
        add(value);
...
class SimpleHash {//这玩意相当于C++中的结构体
    ...
    private int cap;//容量
    private int seed;//种子变量，一个种子对应一个hash工具
    ...
    public int hash(String value) {//字符串哈希，选取好的哈希函数很重要
        ...
        return (cap - 1) & result;//按位与，cap-1只最高位为0，其他位为1。
    }
}