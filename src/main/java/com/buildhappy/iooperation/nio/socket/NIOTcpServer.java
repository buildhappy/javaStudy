package com.buildhappy.iooperation.nio.socket;
...
public class NIOTcpServer {
    ...
    public void listen() throws IOException {
    	...
    	while(true){
    		...
    		while(ite.hasNext()){
    			SelectionKey selectorKey = ite.next();
    			ite.remove();
    			//客户端请求连接事件
    			if(selectorKey.isAcceptable()){
    				ServerSocketChannel server = (ServerSocketChannel) selectorKey.channel();
    				//获得和客户端连接的通道
    				SocketChannel channel = server.accept();
    				//设置成非阻塞
    				channel.configureBlocking(false);
    				//在这里可以给客户端发送信息
    				channel.write(ByteBuffer.wrap("来自server的消息".getBytes()));
